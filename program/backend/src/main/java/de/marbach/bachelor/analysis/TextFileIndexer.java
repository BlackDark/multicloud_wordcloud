...
import gate.creole.annic.apache.lucene.index.FilterIndexReader;
...
public class TextFileIndexer {
	...
	public static void main(String[] args) throws IOException {
		...
		*/
		...
		Fields termVectors = reader.getTermVectors(3);
		...
		do {
			...
			System.out.println(next.utf8ToString() + termFreq);
			next = iterator.next();
		} while(next != null);
		...
		while (!s.equalsIgnoreCase("q")) {
			try {
				...
				TermQuery termQuery = new TermQuery(new Term("contents", "ich"));
	...
	public void indexFileOrDirectory(String fileName) throws IOException {
		...
		for (File f : queue) {
			...
			try {
				...
				//doc.add(new Field("test", fr, Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.YES));
